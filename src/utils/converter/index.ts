...
export function convertToCurrency(
  ...
  option?: { format: string; currency: string }
): string {
  if (!value) {
    value = 0;
  }
...
export function convertObjectToString(
  val: { [key: string]: any } | undefined | null,
  isPrettified = false
): string {
  if (!val) {
    return "";
...
/**
 *  Fix amount number report
 * @param number
 * @returns
 */
export function fixAmount(number: number | string): number {
  const n: number = isNumberString(number)
